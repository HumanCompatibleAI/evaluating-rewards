...
def load_multiple_stats(root_dir: str,
                        keys: Iterable[str],
                        cfg_filter: Callable[[Mapping[str, Any]], bool]
                        = lambda cfg: True,
                       ) -> ConfigStatsMapping:
...
def loss_pipeline(stats: ConfigStatsMapping,
...
def get_affine_from_models(env_name: str, paths: Iterable[str]):
  ...
  with imitation_util.make_session():
    for path in paths:
      model = serialize.load_reward("evaluating_rewards/RewardModel-v0",
                                    os.path.join(path, "model"), venv)
      return model.models["wrapped"][0].get_weights()
  return res


def affine_pipeline(stats: ConfigStatsMapping,
                    preprocess: Tuple[PreprocessFn] = (),
                    figsize=(12, 16)):
...
def replace(pattern: str,
            replacement: str) -> Callable[[Iterable[str]], Iterable[str]]:
