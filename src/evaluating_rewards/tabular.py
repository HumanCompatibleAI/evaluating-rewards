...
def random_state_only_reward(n_states: int, n_actions: int,
...
def random_reward(n_states: int, n_actions: int,
...
def shape(reward: np.ndarray, potential: np.ndarray) -> np.ndarray:
  ...
  return (reward
          + potential[np.newaxis, np.newaxis, :]
          - potential[:, np.newaxis, np.newaxis])
...
def summary_comparison(reward1: np.ndarray,
                       reward2: np.ndarray) -> Tuple[float, float, np.ndarray]:
...
def make_shaped_reward(n_states: int, n_actions: int,
...
def experiment_shaping_comparison(n_states: int,
                                  n_actions: int,
  ...
  for reward_nm in reward_noise:
    for pot_nm in potential_noise:
      ...
      potential_delta = potential_difference(-potential,
                                             pot_nm * noised_reward_potential)
