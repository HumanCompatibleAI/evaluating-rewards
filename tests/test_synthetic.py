...
def test_identical(graph: tf.Graph, session: tf.Session,
                   env_kwargs, scale_kwargs, kwargs, rel_upperbound):
...
@common.mark_parametrize_dict("kwargs", dict(common.combine_dicts(
...
def test_clean_affine(helper_affine, kwargs):
  ...
  return helper_affine(total_timesteps=0,
...
def test_pretrain_affine(helper_affine, kwargs):
  ...
  return helper_affine(reward_hids=[32, 32],
                       dataset_potential_hids=[4, 4],
...
@common.mark_parametrize_dict("env_kwargs", ENVIRONMENTS)
@common.mark_parametrize_kwargs(dict(common.combine_dicts(
    ARCHITECTURES, SYNTHETIC_TEST,
)))
def test_compare_synthetic(graph: tf.Graph, session: tf.Session,
                           rel_upperbound: float, fudge_factor: float,
                           rescale: bool, env_kwargs, kwargs):
